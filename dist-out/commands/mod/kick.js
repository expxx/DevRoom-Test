    ...
    async handle(client, interaction) {
        ...
        if (user.kickable) {
            ...
            const embed = new discord_js_1.EmbedBuilder()
                .setDescription(`\`✅\` <@${interaction.user.id}>, <@${userId}> has been kicked from the server`)
                .setColor('Green')
                .setTimestamp();
            ...
            const embed = new discord_js_1.EmbedBuilder()
                .setDescription(`\`❌\` <@${interaction.user.id}>, I cannot kick <@${userId}>`)
                .setColor('Red')
                .setTimestamp();
            await interaction.reply({ embeds: [embed] });
        }
        ...
        if (!logChannel) {
            return (0, logger_1.fatal)(`User <@${userId}> has been kicked by <@${interaction.user.id}>. No logs channel found.`);
        }
        const channel = logChannel ? interaction.guild?.channels.cache.get(logChannel.logChannel) : null;
        if (!channel || !channel.isTextBased() || !channel.isSendable()) {
            return (0, logger_1.fatal)(`User <@${userId}> has been kicked by <@${interaction.user.id}>. Logs channel not found.`);
        }
        const logEmbed = new discord_js_1.EmbedBuilder()
            .setTitle('User Kicked')
            .setDescription(`User <@${userId}> has been kicked by <@${interaction.user.id}> for \`${reason}\``)
            .setColor('Green')
            .setTimestamp();
