...
export default {
    ...
    async handle(client: Client, interaction: CommandInteraction) {
        ...
        if (!logChannel) {
            return fatal(`User <@${userId}> has been kicked by <@${interaction.user.id}>. No logs channel found.`);
        }

        const channel = logChannel ? interaction.guild?.channels.cache.get(logChannel.logChannel) : null;
        if (!channel || !channel.isTextBased() || !channel.isSendable()) {
            return fatal(`User <@${userId}> has been kicked by <@${interaction.user.id}>. Logs channel not found.`);
        }
